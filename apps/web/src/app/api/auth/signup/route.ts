...
export async function POST(request: Request) {
  try {
    ...
    logger.info(`[API/Auth/SignUp] Attempting server-side signup for: ${email} with role: ${role}`);
    ...
    if (error) {
      logger.error("[API/Auth/SignUp] Sign up failed", error);
    ...
    logger.info(`[API/Auth/SignUp] Sign up successful for user: ${data.user?.id}`);
    return finalResponse;
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Internal server error";
    logger.error("[API/Auth/SignUp] Unexpected error", err);
